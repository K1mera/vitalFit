...
const Filter = () => {
  ...
  const handleSearchPrices = () => {
    if (
      !regexSoloNumeros.test(prices.minPrice) ||
      !regexSoloNumeros.test(prices.maxPrice)
    ) {
  ...
  return (
    <div className="mt-2 font-bebas">
      <div className="h-40 flex flex-col items-center w-[330px] bg-primaryLight rounded-xl p-5">
        <h3 className="text-2xl mb-2">Buscar por nombre</h3>
        <input
          className="mb-2 rounded border-none w-60 placeholder:text-center"
          type="search"
      ...
      <div className="mt-4 flex flex-col justify-start w-full bg-primaryLight rounded-xl p-5">
        ...
        {error && (
          <div className="flex justify-center text-red-600 mt-3">{error}</div>
        )}
