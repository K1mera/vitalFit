  ...
  return (
    <main className="flex flex-col gap-5 h-full">
        ...
        <section className="flex flex-col gap-2">
            {userPaginate.map((user, index) =><UserCardComp key={index} user={user} /> )}  
        </section>
        </div>
        <div className="h-[20%] flex items-end justify-center">

        <section className="flex justify-center items-end gap-4">
            <button value="prev" onClick={handleChangePage} disabled={currentPage <= 1} className=" disabled:opacity-25">{"<"}</button>
            <span>Page {currentPage} / {pages}</span>
            <button value="next" onClick={handleChangePage} disabled={currentPage===pages} className=" disabled:opacity-25">{">"}</button>
        </section>
        </div>
